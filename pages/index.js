...
export default function Home() {
  ...
  const carouselNewItems = [
    ...
    },
    {
      ...
      subtitle:
        "با جسنجو در دسته بندیها، کتابهایی را با هر موضوع و پسندی بیابید.",
    ...
    },
    {
      ...
      subtitle:
        "از برادران کارامازوف تا زنان زیرک و هزاران کتاب خردمندانه ی دیگر که در پایگاه عظیم بست بوک یافت می شوند.",
      backgroundColor: "#cd6133",
      src: "./assets/carousel-photos/mock3.jpg",
    },
  ];

  const advertiseItems = AdvertisementCarouselItems({
    items: carouselNewItems,
  });

  const homeListsEls =
    homeLists.lists["HOME_LISTS"] &&
    homeLists.lists["HOME_LISTS"].map((name, i) => (
      <List
        key={i}
        view={"row"}
        ...
        books={[...books].filter((book) =>
          homeLists.lists[name].books_ids.includes(book.id)
        )}
      />
    ));
  ...
  const homePage =
    selectedBook != "" ? (
      <List
        view={"column"}
        title={""}
        books={[...books].filter((book) => book.id == selectedBook)}
        information
      />
    ) : (
      <React.Fragment>
        ...
        <div className={styles["home__carousel__wrapper"]}>
          <Carousel items={advertiseItems} maxWidth={"80vw"} />
        </div>
        <div className={styles["home__lists"]}>{homeListsEls}</div>
      </React.Fragment>
    );
  return (
